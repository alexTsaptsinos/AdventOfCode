    ...
    if line[0] == "toggle":
        for i in range(numbers[0],numbers[2]+1):
            for j in range(numbers[1],numbers[3]+1):
                matrix[i:i+1,j:j+1] = int((matrix[i:i+1,j:j+1] - 1)**2)
    elif line[1] == "on":
        matrix[numbers[0]:numbers[2]+1,numbers[1]:numbers[3]+1] = 1
    elif line[1] == "off":
        matrix[numbers[0]:numbers[2]+1, numbers[1]:numbers[3]+1] = 0


# Now count number of 1s
total = numpy.count_nonzero(matrix)
